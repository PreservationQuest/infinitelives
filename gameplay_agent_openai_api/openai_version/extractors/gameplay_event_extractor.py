"""Gameplay Event Extractor - OpenAI API version."""
...
class GameplayEventExtractor:
    def __init__(self, api_key=None, model="gpt-4o-2024-08-06", max_retries=3):
        if OpenAI is None:
            self.client = None; logger.warning("openai package not installed. LLM extraction disabled.")
        else:
    ...
    def _call_llm(self, game_title, section, text):
        if self.client is None: logger.warning("No OpenAI client. Skipping."); return None
        ...
        for attempt in range(self.max_retries):
            try:
                response = self.client.chat.completions.create(model=self.model, messages=[{"role": "system", "content": EXTRACTION_SYSTEM_PROMPT}, {"role": "user", "content": user_prompt}], temperature=0.1, max_tokens=2000, response_format={"type": "json_object"})
