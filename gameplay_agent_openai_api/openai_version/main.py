...
from config import OPENAI_API_KEY, OPENAI_MODEL, LOG_LEVEL
...
def cmd_demo(args):
    ...
    if not OPENAI_API_KEY:
        print("\nNote: OPENAI_API_KEY not set. Running with MDA fallback only.\n")
        agent = GameplayEventAgent(openai_api_key="demo-key")
        ...
        return
    agent = GameplayEventAgent(openai_api_key=OPENAI_API_KEY, model=OPENAI_MODEL)
...
def main():
    ...
    if args.command == "demo": cmd_demo(args)
        ...
        if not OPENAI_API_KEY: print("Error: OPENAI_API_KEY not set."); sys.exit(1)
        agent = GameplayEventAgent(openai_api_key=OPENAI_API_KEY, model=OPENAI_MODEL)
